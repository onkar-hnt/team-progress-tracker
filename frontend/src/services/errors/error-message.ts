import { wasAnnouncedGlobally } from '@services/api/api-failure'
...
/**
 * The sentence a feature should show, or `null` when the API layer has already
 * shown one for this failure.
 *
 * Use this wherever a caught error becomes a snackbar. An offline gateway or a
 * 5xx is announced globally, with a message about what actually happened;
 * adding "The task could not be saved" underneath it says nothing new twice.
 * Business refusals — 400, 403, 404, 409 — are not announced globally, so they
 * come back from here as usual.
 */
export function toFeatureMessage(error: unknown, fallback: string): string | null {
  if (wasAnnouncedGlobally(error)) return null
