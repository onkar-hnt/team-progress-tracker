...
public sealed class TeamAccessScopeProvider(TeamDbContext context, ICurrentUser currentUser) : IAccessScopeProvider
{
    ...
    private async Task<AccessScope> BuildMentorScopeAsync(CancellationToken cancellationToken)
    {
        ...
        {
            var assigned = await context.MentorAssignments
                .AsNoTracking()
