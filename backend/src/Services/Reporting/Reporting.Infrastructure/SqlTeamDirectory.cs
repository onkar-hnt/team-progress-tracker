...
/// <summary>
/// Read-only roster lookups for names and mentor assignments. Same cross-schema
/// trade Identity documents for IRosterGateway: one database, narrow SQL, names
/// from <see cref="Db"/>.
/// </summary>
public sealed class SqlTeamDirectory(ReportingConnectionFactory connections) : ITeamDirectory
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await ScalarAsync(sql, cancellationToken, ("@id", developerId));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await ScalarAsync(sql, cancellationToken, ("@id", projectId));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await ScalarAsync(sql, cancellationToken, ("@id", mentorId));
        ...
        CancellationToken cancellationToken = default)
    {
        const string sql = $"""
            SELECT m.Id, m.Name, m.ProfileId
              ...
              JOIN [{Db.Team}].[{Db.Mentors}] m ON m.Id = a.MentorId AND m.DeletedAt IS NULL
